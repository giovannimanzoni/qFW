...
class UtDate
{
    ...
    {
        $ret='';
        if(is_string($mysqlDatetime)) if(  ($mysqlDatetime!='') && (!UtString::areEqual($mysqlDatetime,'0000-00-00 00:00:00'))) {
    ...
    public static function checkDateRange(string $dataIniziale, string $dataFinale): bool
    {
        ...
        if($dateEnd>$dateStart) $esito = true;
    ...
    public static function getDateMax($end)
    {
        ...
        if(is_null($end)) $dateMax = date_create(date('Y-m-d H:i:s'));
        elseif(is_string($end)) {
            $data=str_replace('/', '-', $end);
            $dateMax=date_create($data);
        }
        else{ die('timeLaps $end non nulla ne stringa'); /*@fixme throw error */}
        return $dateMax;
    }
    ...
    public static function timeLaps(string $init,$end, int $secTrigger): string
    {
        ...
        if($giorni>1) $tempo="$giorni giorni e ";
        elseif($giorni==1) $tempo='1 giorno e ';
        else $tempo='';
