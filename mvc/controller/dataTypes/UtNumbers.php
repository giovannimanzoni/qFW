...
class UtNumbers
{
    ...
    public static function getCleanFloat($mixed): float
    {
        ...
        elseif(is_numeric($mixed)) {
            if (!is_float($mixed)) $cleanFloat = floatval($mixed);
            else $cleanFloat=$mixed;
        }
        else {
            if($mixed===false) $str='boolean false';
            else if($mixed===true) $str='boolean true';
            else $str=$mixed;
    ...
    public static function getCleanBool($mixed): bool
    {
        ...
        if(is_bool($mixed)) $cleanBool=$mixed;
        else{
            $int=self::getCleanInt($mixed);
            if($int==1) $cleanBool=true;
            elseif($int==0) $cleanBool=false;
            else throw new \Exception( 'Not boolean argument.');
        }
