...
class Query
{
     ...
     * @return array
     */
    public function fetchKeyPairWhere(string $key, string $value, string $table, string $orderby='2', string $whereColName, $whereColValue): array
    {
        $data=array();
    ...
    public function getValueWhere(string $tbl, string $select,  $whereIdName)
    {
        ...
        try{
            if (is_numeric($whereIdName)) $sql = 'SELECT '.self::formatIdentifier($select).' from '.self::formatIdentifier
                ($tbl).' WHERE `id` = :where LIMIT 1';
            else                    $sql = 'SELECT '.self::formatIdentifier($select).' from '.self::formatIdentifier($tbl).' WHERE `nome` = :where LIMIT 1';

            $result = $this->pdo-> prepare($sql);

            if (is_numeric($whereIdName)) $result-> bindValue(':where', $whereIdName, \PDO::PARAM_INT);
            else                    $result-> bindValue(':where', $whereIdName, \PDO::PARAM_STR);
            $result->execute();
        ...
        if ($str===FALSE)
            $str=''; // se non trova riferimenti, ritorna stringa vuota
        ...
        if(is_numeric($str)){
            ...
            if(UtString::areEqual($tryFloat,$tryInt)) {
                ...
                $tmp=$str.'';
                if($tmp[0]!='0')$str=intval($str);
            }
            else $str=floatval($str);
        }
    ...
    {
        try{
            if (is_numeric($whereIdName)) $sql = 'SELECT DATE_FORMAT('.self::formatIdentifier($select).',"'.$format.'") from '.self::formatIdentifier($tbl).' WHERE `id` = :where LIMIT 1';
            else                    $sql = 'SELECT DATE_FORMAT('.self::formatIdentifier($select).',"'.$format.'") from '.self::formatIdentifier($tbl).' WHERE `nome` = :where LIMIT 1';
        ...
        if(is_numeric($str)){
            $tryFloat=floatval($str);
            $tryInt=intval($str);
            if(UtString::areEqual($tryFloat,$tryInt)) $str=intval($str);
            else $str=floatval($str);
        }
    ...
    public function updateValueWhere(string $tbl, string $colDestinazione,  $rigaRiferimento, $valore): bool
    {
        ...
        try{
            ...
            if (is_numeric($rigaRiferimento)) $sql = 'UPDATE '. self::formatIdentifier($tbl). ' SET ' . self::formatIdentifier($colDestinazione) . " = '$valore' WHERE `id` = :where LIMIT 1";
            else                    $sql = 'UPDATE '. self::formatIdentifier($tbl). ' SET ' . self::formatIdentifier($colDestinazione) . " = '$valore' WHERE `nome` = :where LIMIT 1";

            $result = $this->pdo-> prepare($sql);

            //TODO: penso sia una cagata questa... fare il bind in base al tipo di dato.. dovrei fare il bind a priori.
            if (is_numeric($rigaRiferimento)) $result-> bindValue(':where', $rigaRiferimento, \PDO::PARAM_INT);
            else                    $result-> bindValue(':where', $rigaRiferimento, \PDO::PARAM_STR);
    ...
    public function getStarWhere(string $tbl, string $whereName, $whereValue, $orderby=''): array
    {
        ...
        try{
            $sql = 'SELECT * from '. self::formatIdentifier($tbl) . ' WHERE '. self::formatIdentifier($whereName) . " = $whereValue";
    ...
    {
        $sum=0;
        $sql = 'SELECT ROUND(SUM(   cast(REPLACE('.self::formatIdentifier($column).', ",", ".") as decimal(19,2))   ),2) from '.self::formatIdentifier($tbl);
